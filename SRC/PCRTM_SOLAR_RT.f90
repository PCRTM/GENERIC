...
contains
  ...
  subroutine PCRTM_FORWARD_RT_M_Solar ( RT_SOLUTION,     &
    ...
    real*4, allocatable :: SolarRadup(:),SolarRadup_i(:),SolarRadup_w(:)
    real*8   ::  de_i, de_w, ctau_i, ctau_w
    real*8   ::  mu0,view_mu,albedo, tau
    
    Type(PCRTM_SOLAR_DEF) :: SOLAR_SOLUTION_I, SOLAR_SOLUTION_W
    ...
    allocate(SolarRadup(pcrtm_stnd%nM - StartWaveIndex + 1),   &
             SolarRadup_i(pcrtm_stnd%nM - StartWaveIndex + 1), &
             SolarRadup_w(pcrtm_stnd%nM - StartWaveIndex + 1)) 
    ...
    DO iwave = 1, pcrtm_stnd%nM - StartWaveIndex + 1
       ...
       SolarRadUp(iwave) = 1000.0*mu0*SOLAR_SOLUTION%SolarSpectrum(iwave)*R_TOA/pi
    end DO

!!!! Assign LUTs according to cloud type.
    IF (NCLD .eq. 0) then

       SOLAR_SOLUTION%SolarRadUp = SolarRadUp
    ...
    elseif (NCLD .eq. 1) then
       ...
       pcld = CLD(1)%pcld          
       phase= CLD(1)%phase
       call PCRTM_1CLD_Solar_scatter( RT_SOLUTION,      & 
                                      PCRTM_Stnd,       &
                                      CTAU,             &
                                      DE,               &
                                      PCLD,             &
       ...
       pcld_w = 0
       do n = 1, ncld
          if(CLD(n)%phase .eq. 1) then
             Ctau_i = Ctau_i + CLD(n)%vistau
             De_i   = De_i   + CLD(n)%de*CLD(n)%vistau
             !pcld = pcld + log(CLD(n)%pcld)*CLD(n)%vistau
             pcld_i = pcld_i + CLD(n)%pcld*CLD(n)%vistau
          else if (CLD(n)%phase .eq. 2) then
             Ctau_w = Ctau_w + CLD(n)%vistau
             De_w   = De_w   + CLD(n)%de*CLD(n)%vistau
             !pcld = pcld + log(CLD(n)%pcld)*CLD(n)%vistau
             pcld_w = pcld_w + CLD(n)%pcld*CLD(n)%vistau
          end if
       end do
       !pcld = exp(pcld/(Ctau+1.0e-20))
       pcld_w = pcld_w/(Ctau_w+1.0e-20)
       De_w   = De_w/(Ctau_w+1.0e-20)
       pcld_i = pcld_i/(Ctau_i+1.0e-20)
       De_i   = De_i/(Ctau_i+1.0e-20)
       
       Ctau = ctau_i
       DE   = De_i
       Pcld = Pcld_i
       if (Ctau .lt. 1e-2) then
          ctau = 0.001
          pcld = 100
          De   = 20.0
       end if

       write(*,*) 'ice cloud -----------', ctau, de, pcld
       phase = 1
       ...
       SolarRadup_i = SOLAR_SOLUTION%solarRadup - SolarRadup

       write(*,*) 'calculating ice cloud -------------------'

       
       if (Ctau_i .lt.2.0 .and. Ctau_w.gt.0.0005 ) then
          ...
          write(*,*) 'water cloud -----------', ctau, de, pcld
          ...
          SolarRadup_w = SOLAR_SOLUTION%solarRadup - SolarRadup    
          
          weight = de_w/(de_i+de_w)
          !weight = 0.0
          SOLAR_SOLUTION%SolarRadup = SolarRadup + weight*SolarRadup_i + (1-weight)*SolarRadup_w
          
          write(*,*) 'calculating water cloud -------------------'
    ...
    deallocate(SolarRadup, SolarRadup_i, SolarRadup_w)
  ...
  subroutine PCRTM_1CLD_Solar_scatter( RT_SOLUTION,     &
                                       PCRTM_Stnd,      &
                                       CTAU,            &
                                       DE,              &
                                       PCLD,            &
    ...
    real*8,                            intent(in)    :: CTAU
    real*8,                            intent(in)    :: DE
    real*4,                            intent(in)    :: PCLD

    integer  ::  StartWaveIndex, I1,ncld, L
    real*4   ::  wavenumber, tau_above, tau_below, tau
    real*8   ::  view_mu, mu0, albedo

    DO L= 1, PCRTM_STND%NLAY
       IF ( PCRTM_STND%PBND(L+1) .gt. pcld) THEN
          I1  = L
          exit
       ENDIF
    END DO
    ...
    IF ( I1 .gt.1 ) then
       DO IM = 1, PCRTM_STND%nM
          CALL LINXLINP(0.0,RT_SOLUTION%TAULAY(IM,I1),log(PCRTM_STND%PBND(I1-1)),&
               log(PCRTM_STND%PBND(I1)),alog(PCLD),TAU)
          RT_SOLUTION%TAUGAS_ABOVE(im) = sum(RT_SOLUTION%TAUlay(IM,1:I1-1))+TAU
          RT_SOLUTION%TAUGAS_BELOW(im) = sum(RT_SOLUTION%TAUlay(IM,I1:PCRTM_STND%NLAY))-TAU
       END DO
    END IF
